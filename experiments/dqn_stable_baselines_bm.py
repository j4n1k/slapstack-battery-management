...
def denormalize_action(y: float):
    target_range = (-1, 1)
    original_range = (40, 80)
    c, d = target_range
    a, b = original_range
...
if __name__ == '__main__':
    log_dir_init = './result_data_charging_dqn/init'
    log_dir = './result_data_charging_dqn'
    logfile_name = f'DQN_crossstacks'
    # base_path = "../1_environment/slapstack/slapstack/use_cases/wepastacks_bm/"
    # layout_path = base_path + "1_layout.csv"
    # layout = pd.read_csv(
    #     layout_path,
    #     header=None, delimiter=",")
    # layout.dropna(axis=1, how='all', inplace=True)
    # cleanup_layout("wepastacks_bm")
    # n_cs = 2
    # gen_layout(layout, n_cs)
    ...
    model.learn(total_timesteps=10_000_000, progress_bar=False, log_interval=1,
                ...
                tb_log_name="R1_" + output_string)
    model.save(f"./model_r1_" + output_string + ".zip")

    # model = model.load(f"./model_r1_" + output_string + ".zip")
    ...
    #                 print_freq=100,
    #                 log_dir=f'./result_data_charging')
